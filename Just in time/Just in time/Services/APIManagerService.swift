...
class APIManagerService {
    ...
    public func makeAPICall(originLat: CLLocationDegrees, originLong: CLLocationDegrees,
        ...
        let request = NSMutableURLRequest(url: NSURL(string: "https://ichack-22.herokuapp.com/call?origin=\(originLat),\(originLong)&destination=\(destinationLat),\(destinationLong)&timeBeforeAlarm=\(timeBeforeAlarm)&deviceTime&speed=\(speed)&phoneNumber=\(UserDefaults.getPhoneNumber() ?? "+0")&smsNumbers=\((friendNumbers.map{$0}).joined(separator: ","))")! as URL,
